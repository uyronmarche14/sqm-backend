...
export class AuthRepository extends BaseRepository {
    ...
    async markPasswordResetTokenUsed(passwordResetTokenId) {
        await db.updateTable('PASSWORD_RESET_TOKENS')
            .set({
            used_at: new Date(),
            last_update: new Date(),
    ...
    async findAssignedSqmpAccessibleForms(userId) {
        const accessibleForms = new Set();
        const cycle1Assignments = await sql `
    ...
    `.execute(db);
        ...
        const cycle2Assignments = await sql `
      SELECT TOP 1 1 AS has_access
      FROM SQMP s
    ...
    `.execute(db);
        ...
        const issuerAssignments = await sql `
    ...
    `.execute(db);
        ...
        const supplierResponseAssignments = await sql `
      SELECT DISTINCT form_id
      FROM (
        SELECT 'SQMP-09-06' AS form_id
        FROM SQMP s
    ...
    async findAssignedNpiAccessibleForms(userId) {
        const accessibleForms = new Set();
        ...
        const approvalAssignments = await sql `
      SELECT TOP 1 1 AS has_access
      FROM NPI_LOTS n
    ...
    `.execute(db);
        ...
        const rejectedAssignments = await sql `
      SELECT TOP 1 1 AS has_access
      FROM NPI_LOTS n
    ...
    `.execute(db);
        ...
        const lotTrackingAssignments = await sql `
      SELECT TOP 1 1 AS has_access
      FROM NPI_LOTS n
    ...
    async findAssignedOgiAccessibleForms(userId) {
        const accessibleForms = new Set();
    ...
    async findAssignedMnrAccessibleForms(userId) {
        const accessibleForms = new Set();
        const cycle1Assignments = await sql `
      SELECT TOP 1 1 AS has_access
      FROM MNR_LOTS l
    ...
    async findAssignedQmqaAccessibleForms(userId) {
        const accessibleForms = new Set();
        const cycle1Assignments = await sql `
      SELECT TOP 1 1 AS has_access
      FROM QMQA q
    ...
    async findAssignedQmqaMediaAccessibleForms(userId) {
    ...
    async findAssignedSqprAccessibleForms(userId) {
        const accessibleForms = new Set();
        const draftAssignments = await sql `
      SELECT DISTINCT form_id
      FROM (
    ...
    async findAssignedFiveM1EAccessibleForms(userId) {
        const accessibleForms = new Set();
        const rows = await sql `
      SELECT
