...
export class SqmpService {
    async generateControlNo(fiscalYear, semester) {
    ...
    async getRecordById(id) {
        ...
        if (!data)
            throw new NotFoundError('SQMP Record not found');
    ...
    async updateRecord(id, payload, userId, files = []) {
        ...
        if (payload.registration_date)
            dbUpdates.registration_date = this.parseDate(payload.registration_date);
        if (payload.site_id)
        ...
        if (payload.semester)
            dbUpdates.semester = this.toDBSemester(payload.semester);
        if (payload.issued_date !== undefined)
            dbUpdates.issued_date = this.parseDate(payload.issued_date);
        if (payload.due_date)
        ...
        if (payload.main_document_remarks !== undefined)
            dbUpdates.main_document_remarks = payload.main_document_remarks;
        if (payload.appendix_sheet_remarks !== undefined)
            dbUpdates.appendix_sheet_remarks = payload.appendix_sheet_remarks;
