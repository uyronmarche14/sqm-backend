...
import { qmqaScheduleSchema, qmqaAuditPlanSchema, qmqaAuditDetailsSchema, qmqaResponseSchema, qmqaVerificationSchema, qmqaFinalReportSchema } from '../schemas/qmqa.schema.js';
const router = express.Router();
// ==================== SCHEDULE ENDPOINTS ====================
// All schedule endpoints require authentication
router.post('/schedules', authenticateToken, validateRequest({ body: qmqaScheduleSchema }), asyncHandler(qmqaController.createSchedule));
...
router.put('/schedules/:id', authenticateToken, canModifyRecord, validateRequest({ body: qmqaScheduleSchema }), asyncHandler(qmqaController.updateSchedule));
...
router.post('/records', authenticateToken, validateRequest({ body: qmqaAuditPlanSchema }), asyncHandler(qmqaController.createRecord));
...
router.put('/records/:id', authenticateToken, canModifyRecord, validateRequest({ body: qmqaAuditDetailsSchema }), asyncHandler(qmqaController.updateRecord));
...
router.post('/response/:token/initial', validateRequest({ body: qmqaResponseSchema }), asyncHandler(qmqaController.saveInitialReport));
router.post('/response/:token/final', validateRequest({ body: qmqaFinalReportSchema }), asyncHandler(qmqaController.submitFinalReport));
// ==================== VERIFICATION AND CYCLE 2 ENDPOINTS ====================
router.post('/records/:id/verification', authenticateToken, canModifyRecord, validateRequest({ body: qmqaVerificationSchema }), asyncHandler(qmqaController.submitVerification));
