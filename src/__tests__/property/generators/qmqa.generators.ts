...
import type { QMQAStatus } from '../../../utils/qmqa-business-logic.js';

/**
 * Arbitrary generator for QMQA status values
 * Generates all 12 possible status values in the QMQA workflow
 */
export const qmqaStatusArbitrary = fc.constantFrom<QMQAStatus>(
  'PLANNED',
  'DRAFT',
  'AWAITING_APPROVAL',
  'REJECTED',
...
export const controlNoArbitrary = fc.oneof(
  ...
  ).map(([year, seq]) => `P-${year}-${String(seq).padStart(3, '0')}`),
  
  // Direct format: YYYY-NNN
  fc.tuple(
    fc.integer({ min: 2020, max: 2030 }),
    fc.integer({ min: 1, max: 999 })
  ).map(([year, seq]) => `${year}-${String(seq).padStart(3, '0')}`)
);
...
export const qmqaScheduleArbitrary = fc.record({
  ...
  status: fc.constant<QMQAStatus>('PLANNED'),
  createdBy: fc.uuid(),
...
export const qmqaRecordArbitrary = fc.record({
  ...
  actualDate: fc.option(
    fc.date({ min: new Date('2020-01-01'), max: new Date('2030-12-31') })
      .map(d => d.toISOString().split('T')[0]),
    { nil: undefined }
  ),
  issuedDate: fc.option(
    fc.date({ min: new Date('2020-01-01'), max: new Date('2030-12-31') })
      .map(d => d.toISOString().split('T')[0]),
    { nil: undefined }
  ),
