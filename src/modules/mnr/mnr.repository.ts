...
export class MnrRepository extends BaseRepository<'MNR_LOTS'> {
  ...
  async findAllDetailed(statusFilter?: string | string[]) {
    let query = db.selectFrom('MNR_LOTS as l')
      ...
      .select([
        ...
        sql<string | null>`${sql.ref('st.site_code')}`.as('site_code'),
        ...
        sql<string | null>`${sql.ref('dc.defectcategory_acronym')}`.as('defectcategory_acronym'),
  ...
  async findAttachmentOwner(attachmentId: string): Promise<{ moduleType: 'mnr-main' | 'mnr-response'; mnrId: string } | null> {
