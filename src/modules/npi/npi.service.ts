...
export class NpiService {
  ...
  async createRecord(payload: NPICreationInput, userId: string, files: any[] = []) {
    const npiId = uuidv4();
    const now = new Date();
  ...
  async updateRecord(id: string, payload: NPIUpdateInput, userId: string, files: any[] = []) {
    ...
    if (payload.deliveryDate) dbUpdates.delivery_date = this.parseDate(payload.deliveryDate);
