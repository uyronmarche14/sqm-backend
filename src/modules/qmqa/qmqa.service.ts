...
export class QmqaService {
  ...
  async createRecord(payload: QMQARecordCreationInput, userId: string, files: any[] = []) {
    ...
    return await qmqaRepository.executeTransaction(async (trx) => {
      ...
      let resolvedAttentionId: string | null = cleanAttentionId;
      if (cleanAttentionId) {
  ...
  async updateRecord(id: string, payload: QMQARecordUpdateInput, userId: string) {
    ...
    if (!currentRecord) throw new NotFoundError('QMQA Record not found');
    const apid = currentRecord.qmqa_audit_plan_id;

    // Resolve attention_id if provided
    let resolvedAttentionId: string | null = undefined as any;
